...
xx = xx (:);
yy = yy(:);   % stretch 2D grids to 1D vectors
f = 10 * sin(8*xx.*(yy-1));
D2 = D^2;
D2 = D2(2:N,2:N);
I = eye(N-1);
...
% Interplolte to finear grid and plot:
[xxx, yyy] = meshgrid(-1:0.01:1, -1:.01:1);
uuu = interp2(xx, yy, uu, xxx, yyy, 'cubic');
figure(2), clf, mesh(xxx, yyy,uuu), colormap([0.6 0.3 0.2])
